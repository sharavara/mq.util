...
public class BrowseMessages extends SwingWorker<Integer, Void> {
	...
	public BrowseMessages(JButton btnBrowse, JProgressBar progressBar, JTable table, MQProvider mq, DefaultTableModel tModel, String qname, JTextArea textLog, Integer mLimit) {
		this.btn = btnBrowse;
	...
	protected Integer doInBackground() throws Exception {
		...
		try {
			...
			HashMap mesObj = mq.browseMessage(qname, MQC.MQGMO_BROWSE_FIRST);
			MQMessage message = new MQMessage();
			int i = 1;
			//System.out.println("mLimit:" + mLimit );
			while (mesObj != null && mLimit >= i) {
				...
				if (mLimit > i-1) 
					mesObj = mq.browseMessage(qname, MQC.MQGMO_BROWSE_NEXT);
			}
			...
			textLog.append(datetimeFormat.format(new Date()) + "\tMessages have been browsed\n");
		} catch (Exception ex) {
			ex.printStackTrace();
